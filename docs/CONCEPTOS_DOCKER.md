...
| `EXPOSE 8021` | **Documenta** en qué puerto escucha el programa | No abre nada: quien publica el puerto es el `ports:` del compose |
...
- En la v1: `bd_v1` es un contenedor creado desde la imagen `postgres:16-alpine`,
  con el puerto interno 5432 **publicado** en el 15439 de su PC (`-p 15439:5432`).
...
flowchart LR
    ...
    subgraph PC["Su PC — Docker Desktop (el 'centro de datos')"]
        subgraph RED["red interna del compose (LAN virtual, con DNS propio)"]
            FRONTFLASK["SERVIDOR WEB (front)<br/>contenedor front-flask<br/>hostname: front-flask · escucha en 8023"]
            APIFACTURAS["SERVIDOR DE APLICACIONES<br/>contenedor api-facturas<br/>hostname: api-facturas · escucha en 8021"]
    ...
    NAV -->|"localhost:8021"| APIFACTURAS
    NAV -->|"localhost:8023"| FRONTFLASK
    ...
    FRONTFLASK -->|"http://api-facturas:8021<br/>al controlador, por el NOMBRE"| APIFACTURAS
    SQLSERVERINIT -->|"espera el healthcheck,<br/>siembra y termina"| SQLSERVER
    CLIENTE -.->|"opcional (diagnóstico):<br/>localhost:13338"| MARIADB
    CLIENTE -.->|"opcional (diagnóstico):<br/>localhost:15439"| POSTGRES
    CLIENTE -.->|"opcional (diagnóstico):<br/>localhost:11438"| SQLSERVER
```
...
services:                          # el mapa de TODOS los contenedores del sistema
  ...
  postgres:                        # ← este nombre es también su HOSTNAME interno
    ...
    ports:
      - "15439:5432"               # "puerto en su PC : puerto interno del contenedor"
  ...
  api-facturas:
    ...
    ports:
      - "8021:8021"                # http://localhost:8021/docs
    environment:
...
1. **Dos redes de nombres**: hacia su PC, puertos publicados
   (`localhost:8021`, `localhost:15439`); entre contenedores, nombres de
...
docker network create proyecto_paradigmas7_default

# 2. postgres
docker run -d --name postgres --network proyecto_paradigmas7_default --restart unless-stopped `
...
# 4. mariadb
docker run -d --name mariadb --network proyecto_paradigmas7_default --restart unless-stopped `
  ...
  -p 13338:3306 mariadb:11

# 5. ESPERAR a que responda de verdad… mirándolo a ojo

# 6. sqlserver
docker run -d --name sqlserver --network proyecto_paradigmas7_default --restart unless-stopped `
  -e "ACCEPT_EULA=Y" `
  -e "MSSQL_SA_PASSWORD=Paradigmas123!" `
  -e "MSSQL_PID=Developer" `
  -v mssqldata:/var/opt/mssql `
  -p 11438:1433 mcr.microsoft.com/mssql/server:2022-latest

# 7. ESPERAR a que responda de verdad… mirándolo a ojo

# 8. sqlserver-init
docker run -d --name sqlserver-init --network proyecto_paradigmas7_default --restart no --entrypoint /bin/bash `
  -e "MSSQL_SA_PASSWORD=Paradigmas123!" `
...
docker run -d --name api-facturas --network proyecto_paradigmas7_default --restart unless-stopped `
  ...
  -p 8021:8021 api-facturas uvicorn main:app --host 0.0.0.0 --port 8021 --reload

# 10. Construir la imagen de front-flask y encenderla
docker build -t front-flask ./front_flask
docker run -d --name front-flask --network proyecto_paradigmas7_default --restart unless-stopped `
  -e "API_FACTURAS_URL=http://api-facturas:8021" `
  -e "CLAVE_SESION=Paradigmas123!Sesion" `
  -v "${PWD}/front_flask:/app" `
  -p 8023:8023 front-flask
...
```yaml
URL_API: http://api-facturas:8021
```
...
|---|---|---|
| **Su navegador** | `http://localhost:8021` | Está fuera de Docker. Usa el puerto **publicado** |
| **Otro contenedor** | `http://api-facturas:8021` | Están en la misma red: se llaman por el **nombre del servicio** |
...
> **En este proyecto los dos números son iguales** (`8021:8021`), y eso
...
Si alguien cambiara esa línea por `http://localhost:8021`, el front
...
```powershell
docker network inspect proyecto_paradigmas7_default
```
...
`docker-compose.yml` y le pega `_default`. La carpeta `proyecto_paradigmas7` produce
la red `proyecto_paradigmas7_default`. Tres formas de confirmarlo:
...
docker inspect proyecto_paradigmas7-front-flask-1 --format '{{range $k,$v := .NetworkSettings.Networks}}{{$k}}{{end}}'
```
...
network proyecto_paradigmas7_default has active endpoints (name:"proyecto_paradigmas7-front-flask-1" …)
```
