"""
Servicio de empresa — la capa de NEGOCIO (v2). CALCADO del de producto:
...
from repositorios.abstracciones.i_repositorio_empresa import IRepositorioEmpresa


class ServicioEmpresa:
    """Reglas de negocio del CRUD de empresa."""

    def __init__(self, repositorio: IRepositorioEmpresa):
    ...
    def _validar_codigo(codigo: str) -> str:
        ...
        if not codigo:
            raise ValueError("El código de la empresa no puede estar vacío.")
        return codigo
    ...
    async def obtener(self, codigo: str) -> dict:
        ...
        if fila is None:
            raise LookupError(f"No existe una empresa con codigo = {codigo}")
        return fila
