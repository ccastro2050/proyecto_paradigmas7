"""
Repositorio de usuario para SQL Server — la capa de DATOS (v7).

CALCADO del repositorio de empresa: SQLAlchemy async como ejecutor, SQL
visible y parametrizado con text().

La única diferencia con sus gemelos es la de siempre: T-SQL no
tiene LIMIT, tiene TOP (:limite) al principio del SELECT.
...
class RepositorioUsuarioSqlServer:
    """Implementación concreta de IRepositorioUsuario contra SQL Server."""
    ...
    async def obtener_todos(self, limite: int) -> list[dict]:
        ...
        sql = text(
            "SELECT TOP (:limite) email FROM usuario ORDER BY email"
        )
